...
export class CrudService {
  ...
  async getUserByMatricula(matricula: string): Promise<AlunoData | null> {
    const ref = doc(this.firestore, `alunos/${matricula}`);
    const snap = await getDoc(ref);
    return snap.exists() ? (snap.data() as AlunoData) : null;
  } 
}
